...
**Solução: a angularidade transborda.** Acima do topo da régua ela não é travada em 1; sobe até `transbordoMax`, com 1,45 como ponto de partida. A régua é calibrada pelo **corpo** da música (percentis 5 e 95); um pico é exceção e precisa parecer exceção. Resultado medido na página: o pico chega a 1,45, a forma passa de 1 em **8% dos quadros**, e a forma grave continua abaixo de 0,41 — as duas nunca se cruzam.

O transbordo é decisão de projeto, declarada. Ele não muda a direção nem a ordem — mais brilho continua mais pontudo, e a forma grave continua sempre abaixo da média-aguda —, só devolve aos picos a altura que a normalização tirava. `transbordoMax = 1,0` desliga.
...
11. **Os picos de brilho aparecem.** A forma média-aguda deve passar de 1 em alguns por cento dos quadros, não em nenhum e não na maioria. Se nunca passar, o transbordo está desligado ou a régua está larga demais; se passar o tempo todo, a calibração está estreita. Medido no áudio de teste: 8%.
...
**P7. A simetria das formas.** As formas são simétricas porque o desenho as fez assim, não porque alguma medida peça: cada forma recebe três escalares, e escalar não tem lado. A autora registrou incômodo leve com isso em 20/09. Três caminhos, e a decisão vem depois de assistir em movimento, não em quadros parados:
- **manter regular** — não existe assimetria, e tudo na tela mede alguma coisa
- **irregularidade decorativa**, derivada da calibração para continuar repetível — fica perto das referências visuais, e o texto declara que a variação entre as pontas não codifica nada
- **três sub-regiões por faixa**, cada uma modulando um terço do contorno — a assimetria passa a significar a desigualdade interna da faixa. Propositivo, mas medido. **Risco a pesar:** levado adiante, isso vira um espectro em coordenadas polares, que a seção 2 exclui. Três números é medida; o espectro inteiro é display. A linha precisa ser defendida no texto
